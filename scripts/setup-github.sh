#!/usr/bin/env bash
# One-time GitHub setup: labels + enable Pages. No secrets are stored on GitHub.
set -euo pipefail
REPO=${REPO:-$(gh repo view --json nameWithOwner --jq .nameWithOwner)}

label() { gh label create "$1" --repo "$REPO" --color "$2" --description "$3" --force; }
label daily          0E8A16 "每日推送 PR"
label to-read        1D76DB "待精读（本地 queue 会处理）"
label reading        5319E7 "精读进行中"
label note           C5DEF5 "精读笔记 PR"
label research-topic D93F0B "待调研主题（本地 queue 会处理）"
